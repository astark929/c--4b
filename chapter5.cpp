...
string printTable() {
    ...
    string output ="";
        if (head == NULL) {
            ...
            int i = 1;
            while (current != NULL && i <= count) {
            table += current->ISBN + "\t\t" + current->title_book + 
            "\t\t" + current->author + "\t\t" + current->usedornot + 
            "\t\t" + current->enrolled + "\t\t" + current->prices_book + 
            "\t\t" + current->required + "\n";
            i++;
            current = current->nextTransaction;
            }
...
// Function to read data from a file and adds the values from the file and puts them into linked list
// please note the file should have at least a tab (\t) otherwise it wont be able to read all the values correctly
...
void readFileData(string filename) {
    ...
    getline(inputFile, line);
        ...
        while (spacePos != string::npos) {
            line.replace(spacePos, 3, "\t");
            spacePos = line.find("   ");
        }
        
        // this code is just here to replace any 3 spaces with a tab since for some reason its doing that for me
        
        spacePos = line.find("  ");  
        while (spacePos != string::npos) {
            line.replace(spacePos, 2, "\t");
            spacePos = line.find("  ");
        }
        // same thing as above but its here incase there is a char value taking up one of the sapaces meant for the tab
...
while (getline(ss, value, '\t')) {
        // Process each extracted value here
        cout << "Value: " << value << endl;
    }
*/ 
//this batch of code above is just a debugging code to make sure it can read all the inputs 

        if (endPos == string::npos) {
            // Handle the case when there are no more tabs in the line
            break;
        }

        
        ISBN = line.substr(startPos, endPos - startPos);
        //cout << ISBN << endl; //more debugging command
        startPos = endPos + 1;
        ...
        endPos = line.find('\t', startPos);
        title_book = line.substr(startPos, endPos - startPos);
        //cout << title_book << endl;
        startPos = endPos + 1;
        

        endPos = line.find('\t', startPos);
        usedornot = line.substr(startPos, endPos - startPos);
        //cout << usedornot << endl;
        startPos = endPos + 1;
        

        endPos = line.find('\t', startPos);
        enrolled = line.substr(startPos, endPos - startPos);
        //cout << enrolled << endl;
        startPos = endPos + 1;
        

        endPos = line.find('\t', startPos);
        required = line.substr(startPos, endPos - startPos);
        //cout << required << endl;
        startPos = endPos + 1;
        

        endPos = line.find('\t', startPos);
        prices_book = line.substr(startPos, endPos - startPos);
        //cout << prices_book << endl;
        startPos = endPos + 1;

        // Repeat the process for other variables...
        Transaction* newTransaction = new Transaction(ISBN, author, title_book, usedornot, enrolled, prices_book, required);
        
        // Call the addTransaction method of the current object (using the this pointer)
        this->addTransaction(newTransaction);
        //cout << "--------------------------------------------------" <<endl; //more debugging to seperate each stored transaction

        if (current != NULL) {
        current->nextTransaction = newTransaction;
        newTransaction->prevTransaction = current;
        current = newTransaction;
        } else {
            head = newTransaction;  // Set the newTransaction as the head if current is NULL
            current = newTransaction;
        }
        ...
        //cout << "adding the transaction were a success" << endl;
    }
    inputFile.close();
    cout << "implementing the values from a file into the list of transactions were successful\n\n" << endl;
}
...
//-----------------------------------------------------------------------------------------------------------------
   ...
   case '3': // search, sort, print, and edit
   {
    ...
    case '4': //adding a file to the object linked list
    {
        string userinput = "";
        cout << "please enter the full file name including file type to access the file (ex. test.txt)" << endl;
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // clears previous inputs that may claug the input status allowing the next input to not be skipped
        std::getline(std::cin, userinput);

        handler.readFileData(userinput);
        cout << "press enter to continue back to the main menu"<< endl;
        cin.get();
        flush();
        main_menu_screen();
    ...
    case '5': //exit screen
    {
        ...
        timelock++;
        break;
        //the 5th case brings the user to the exit screen and terminates the code
    }
